...
pub fn wrap_async_read<R : AsyncRead + Unpin + Send + 'static>(mut read: R) -> BoxStream<'static, ClientRequest>{
   ( stream! {
        loop {
...
async fn read_packet<R : AsyncRead + Unpin>(reader: &mut R) -> ClientRequest {
    let len = reader.read_u64().await.expect("could not read packet length");
    let mut buffer = vec![0_u8; len as usize];
    reader.read_exact(&mut buffer).await.expect("could not fill buffer");
